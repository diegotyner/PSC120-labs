...
public class Environment extends SimStateSweep {
	...
	public void makeAgent() {
		...
		for (int i = 0; i < females; i++) {
			...
			gui.setOvalPortrayal2DColor(f, (float) 1, (float) 0, (float) 0,
					(float) (attractiveness / maxAttractiveness));
			female.add(f);
		}
		for (int i = 0; i < males; i++) {
			...
			gui.setOvalPortrayal2DColor(m, (float) 0, (float) 0, (float) 1,
					(float) (attractiveness / maxAttractiveness));
			male.add(m);
		}
	}
	...
	public void start() {
		...
		makeAgent();
		if (observer != null) {
			observer.initialize(sparseSpace, Spaces.SPARSE);// initialize the experimenter by calling
									// initialize in the
									// parent class
